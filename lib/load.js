...
function* callInit(moduleName, from, modules) {
  ...
  for (let dependencyName in module.dependencies) {
  ...
  Object.keys(module.dependencies).forEach(function (dependencyName) {
    //构建魔术 dependency
    //要让module的extend等方法既能拿到 relier，又要保证操作的数据是原module上的
    let magicDep = { relier: moduleName }
    let dependency = modules[dependencyName]
    _.forOwn( dependency, function( value, name){
      if( typeof value === 'function' || isGenerator(value)){
        magicDep[name] = dependency[name]
      }else{
        Object.defineProperty( magicDep, name, {
          get : function(){
            return dependency[name]
          },
          set : function( newValue ){
            dependency[name] = newValue
          }
        })
      }
    })
    module.deps[dependencyName] = magicDep
  })
