...
function requireModulesByPath( modulePath) {
  ...
  files.forEach(function (fileName) {
    ...
    var moduleName = (info.zero && info.zero.alias) || info.name.replace("zero-","")
    info.zero = info.zero || {}
    info.zero.dependencies = info.zero.dependencies || {}
...
function callInit( moduleName, from, modules, nextModule ){
  ...
  logger.info("loader", "begin init", moduleName)
  from = from || []
...
exports.loadAll = function (opt, loadModulesDone) {
  ...
  async.eachSeries( Object.keys(modules), function( name, nextModule ){
    ...
    if( err ){
      logger.error("call module init failed, due to", err)
      return loadModulesDone(err)
    }

    loadModulesDone( undefined, modules)
  })
}
