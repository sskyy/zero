...
Bus.prototype.fire = function (opt) {
  ...
  if (root.opt.track) {
    ...
    currentEventRef = {
      "name": eventOrg,
      "argv" : [].map.call(arguments,function(a){return Object.prototype.toString.call( a)}),
  ...
  forEachSeries(matchedEventStack, function (matchedEvent, i, nextEvent) {
    ...
    forEachSeries(matchedEvent.listeners, function (listener, j, nextListener) {
      ...
      if( results[listener.name] ){
        if( !_.isArray( results[listener.name]))  results[listener.name] = [results[listener.name]]
...
Bus.prototype.error = function( status, error ){
  ...
  if( this.opt.track) {
    console.log("setting error", busError)
