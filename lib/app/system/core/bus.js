...
Bus.prototype.snapshot = function(options){
  ...
  if( !this._started ){
    logger.log("you can only snapshot started bus")
    return false
  }
...
//    logger.log("[BUS] getting data", name, this.$$data, ref)
  ...
  while( currentName = ns.shift() ){
    if( ns.length == 0 ){
      if( _.isObject(ref[currentName] )){
        ...
        if( ref[currentName] !== undefined ) logger.log("you are changing a exist data",name)
        ref[currentName] = data
      }

    }else{
      if( !_.isObject(ref[currentName])) {
        if( ref[currentName] !== undefined ) logger.log("your data will be reset to an object",currentName)
        ref[currentName] = {}
      }
      ref = ref[currentName]
    }
...
//  logger.log("[BUS] setting done", name, obj)
}
...
Bus.prototype.fire = function (opt) {
  ...
  logger.log("[BUS] firing :", eventOrg, 'bus id:', this._id)
  ...
  forEachSeries(matchedEventStack, function (matchedEvent, i, nextEvent) {
    ...
    forEachSeries(matchedEvent.listeners, function (listener, j, nextListener) {
      ...
      logger.log("[BUS] applying :", eventOrg, listener.module, listener.name)
      ...
      if( res instanceof  BusError ){
        ...
        res.then(function(){nextListener()}).catch(nextListener)
      }else{
        nextListener()
      }
      ...
      if( util.isPromiseAlike(fcallResult) ){
        return fcallResult.then( function( fcallResultResolved){
          return root.fire.apply( root, [eventOrg + ".after" ].concat([fcallResultResolved]).concat(args)).then(function( afterResult){
            logger.log("after ",eventOrg,afterResult)
...
Bus.prototype.error = function( status, error ){
  logger.log("[BUS] error", status, error)
